...
This is worth being precise about, because it determines where the fix can live. The caller
...
RFC 9110 says that `GET`, `HEAD`, `PUT`, `DELETE` and `OPTIONS` are idempotent, while `POST`
and `PATCH` are not. Read carefully, that sentence is a statement about what those methods
are **defined to mean**, not a promise about what any particular server does when you call one
twice.
...
Some operations are idempotent without help, and it is worth knowing which, because they are
the ones you do not need to spend effort on.
...
The unique constraint is the other source of natural protection, and the most reliable one:

```sql
ALTER TABLE payments ADD CONSTRAINT payments_request_id_key UNIQUE (request_id);
```

If the caller supplies an identifier and the database refuses the second insert, the duplicate
cannot produce a second row no matter how many layers above it went wrong. This is a real
technique with real limits, and the limits are the subject of the next article.
...
consumer**", or per handler, or per workflow step. Whatever identifies the unit of work has to
be part of how a duplicate is recognised, because two different pieces of work legitimately
share an event id and must not be treated as the same thing.

Get this wrong in the permissive direction and duplicates slip through. Get it wrong in the
restrictive direction and you have a much worse bug: the second consumer's first delivery is
suppressed as a duplicate, and work that was supposed to happen silently does not. That
failure produces no error, no log line and no alert. It is discovered by a customer.
...
This generalises into a rule worth holding on to: making your own work safe to retry is
...
harmless, which is ordinary engineering.
