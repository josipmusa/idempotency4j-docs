...
public Payment charge(PaymentRequest request) {
    try {
        return paymentRepository.save(new Payment(request.requestId(), request.amount()));
    } catch (DataIntegrityViolationException e) {
        return paymentRepository.findByRequestId(request.requestId()).orElseThrow();
    }
}
```
...
external API, those happen before the insert fails, and the constraint does not undo them. It
is idempotent for the row, not for the method.

**It answers badly.** The duplicate gets an exception you have to catch and turn into
something. For a simple create-and-return that is a few lines. For a handler that did five
things, reconstructing the original response from the one row you constrained is not always
possible.
...
The fix is to **insert first and let the insert fail**:

```java
@Transactional
public void handle(OrderPlaced event) {
    try {
        processedEventRepository.saveAndFlush(new ProcessedEvent(event.id()));
    } catch (DataIntegrityViolationException e) {
