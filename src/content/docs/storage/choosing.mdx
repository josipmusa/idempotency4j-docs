...
<CardGrid>
  <LinkCard
    title="JDBC"
    href={href('/docs/storage/jdbc/')}
    description="Autoconfigured from a DataSource. The only store that can join your transaction."
  />
  <LinkCard
    title="Redis"
    href={href('/docs/storage/redis/')}
    description="Three explicit beans. Tuning, replica acknowledgement, and what it cannot do."
  />
  <LinkCard
    title="In-memory"
    href={href('/docs/storage/in-memory/')}
    description="Development and tests only. One JVM, lost on restart, asked for by name."
  />
  <LinkCard
    title="Writing a store"
    href={href('/docs/storage/writing-a-store/')}
    description="The three-method SPI and the contract that specifies it."
  />
</CardGrid>
