...
- **`keyPrefix`** - namespace for every key the store writes. Defaults to the provider's own
  prefix.
- **`pollInterval`** - how often a waiting caller re-checks a held key. Defaults to 50ms.
- **`retentionGrace`** - extra margin before an expired record is purged. Defaults to
  1 hour.
- **`purgeBatchSize`** - records scanned per purge page. Defaults to 500.
