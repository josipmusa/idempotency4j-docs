...
SpEL expression over the method's parameters because there is no transport to take one from. Its
default scope is the class and method name, which keeps two consumers of the same message
independent. With the JDBC store and `join-transaction` completion, the record is written on the
caller's own transaction-bound connection, so the record and the business writes commit or roll
back together. That is step 1's one-commit property, for the inbound half.

What it does not do today is the rest of the picture. There is no messaging module: nothing
extracts a key from a Kafka record or an SQS message for you, nothing writes an outbox, nothing
dispatches one, and nothing knows what a Spring Modulith event publication is. The one-commit
property belongs to the JDBC store; the Redis store reports that it cannot support transactional
completion and always will, because there is no transaction a JDBC caller's writes could join.
The HTTP side is a servlet filter, with no reactive equivalent.
